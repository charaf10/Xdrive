...
export default function LOGIN({navigation}){
  ...
  const Login_Press = () => {
    ...
    axios.post('http://' + myip +':80/link/entreprise/Login.php', {
     
    Noentreprise: username,
    Password: password,
    ...
    .then((response) => {
        ...
        if (result == "correct") {
            //alert("id: " + resultId);
            navigation.navigate("Entreprise_dash",{id: resultId, username: username, password: password});
    ...
    const button_signUp_entreprise = <Button_sign 
                                title={"SignUp !"} 
                                onPress={() => {
                                navigation.navigate("Signup_entreprise");
                                }}
                                />
    ...
    let leRetour = 
        <View style={{ 
            ...
            <Text>{[button_signUp_entreprise ]}</Text>
        </View>
